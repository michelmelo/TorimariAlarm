var openAlarmEditWin = function(isEdit) {
    ...
    setTimeBtn.addEventListener('click', function() {
        Titanium.API.info(timePicker.getValue());
        Titanium.API.info("time: " + new Date(new Date() - new Date(timePicker.getValue())).getTime());
        Titanium.API.info("conta: " +  (1000 * 60 * 60 * 24));
        Titanium.API.info("contac: " + (timePicker.getValue().setSeconds(0) + 1000 * 60 * 60 * 24));
        if (new Date(new Date() - new Date(timePicker.getValue())).getTime() > -1) {
            // Set 24 hours later if the time is earlier than current time.
            clock.setAlarm(new Date(timePicker.getValue().setSeconds(0) + 1000 * 60 * 60 * 24), currentCheckedVoiceId, snoozeSwitch.value, true);
            Titanium.API.info('added 24 hours');
        } else {
            clock.setAlarm(new Date(timePicker.getValue().setSeconds(0)), currentCheckedVoiceId, snoozeSwitch.value, true);
