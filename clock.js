...
Clock.prototype.sortAlarmList = function() {
	this.alarmList.sort(function(x, y) {return x.date - y.date;});
}
...
Clock.prototype.enableAlarm = function(index) {
  ...
  if (new Date(now - oldDate).getTime() > -1) {
	...
	var alarmObj = this.createAlarmObj(
			newDate, 
			this.alarmList[index].mediaId,
			this.alarmList[index].isSnooze,
			true);
	this.setAlarmTimeOut(alarmObj);
...
Clock.prototype.setBackgroundAlarm = function() {
  ...
  Ti.App.iOS.registerBackgroundService({url: 'bg_alarm.js'});
};
